...
class LinearNode(BaseNode):  # 继承BaseNode
    ...
    def build(self):  # 构建层
        self.linear = nn.Linear(  # 创建线性层
            self.params.get("in_features", 64),  # 输入特征数
            self.params.get("out_features", 64),  # 输出特征数
            bias=self.params.get("bias", False),  # 是否使用偏置
        )
...
class ConvNode(BaseNode):  # 继承BaseNode
    ...
    def build(self):  # 构建卷积层
        ...
        self.conv = convCls(  # 创建卷积层
            in_channels=self.params.get("in_channels", 3),  # 输入通道数
            out_channels=self.params.get("out_channels", 64),  # 输出通道数
            kernel_size=self.parseList(
                self.params.get("kernel_size", "3")
            ),  # 卷积核大小
            stride=self.parseList(self.params.get("stride", "1")),  # 步幅
            padding=self.parseList(self.params.get("padding", "0")),  # 填充
            dilation=self.parseList(self.params.get("dilation", "1")),  # 膨胀
            groups=self.params.get("groups", 1),  # 分组数
