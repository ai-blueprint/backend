...
**使用方式**：
```python
@category(id="basic", name="基础", color="#8B92E5")
def basic_category():
...
```python
# 在关键位置插入调试节点
@node(id="debug", name="调试输出", inputs=["x"], outputs=["out"])
def debug_node():
    def compute(x, layer):
...
参数命名要清晰，默认值要合理：
```python
@node(id="dropout", name="Dropout", inputs=["x"], outputs=["out"],
...
#### 节点文档
```python
@node(id="attention", name="注意力层", inputs=["query", "key", "value"], outputs=["out"],
