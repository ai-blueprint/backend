...
def topoSort(nodes, edges, strict=False):
    ...
    nodeLabels = {}  # 节点id到节点label的映射，用于错误信息增强

    for node in nodes:  # 遍历所有节点
        nodeId = node.get("id", "")  # 获取节点id
        nodeData = node.get("data", {})  # 获取节点data字段，label可能放在这里
        nodeLabel = node.get("label", "")  # 先尝试直接从节点顶层读取label
        if nodeLabel == "" and isinstance(nodeData, dict):  # 如果顶层没有label就尝试从data读取
            nodeLabel = nodeData.get("label", "")  # 从data读取label
        if nodeLabel == "":  # 如果还是没有label
            nodeLabel = nodeId  # 退化为id，避免报错信息为空

        if nodeId in nodeIds:  # 如果节点id已经出现过
            raise Exception(f"存在重复节点ID，无法进行拓扑排序: {nodeId} (label: {nodeLabel})")  # 抛出重复节点错误并附带label，避免图结构歧义
        nodeIds.add(nodeId)  # 记录当前节点id
        nodeLabels[nodeId] = nodeLabel  # 记录节点label供后续错误信息使用
    ...
    for edge in edges:  # 遍历所有边
        ...
        if source not in adjacency:  # 如果源节点不在邻接表中
            if strict:  # 严格模式下直接报错，提示边的源节点非法
                targetLabel = nodeLabels.get(target, "未知")  # 目标节点存在时输出其label，便于快速定位连线
                raise Exception(f"存在非法边，源节点不存在: {source} -> {target} (target label: {targetLabel})")  # 抛出非法边错误并附带目标label
            continue  # 跳过这条边

        if target not in inDegree:  # 如果目标节点不在入度表中
            if strict:  # 严格模式下直接报错，提示边的目标节点非法
                sourceLabel = nodeLabels.get(source, "未知")  # 源节点存在时输出其label，便于快速定位连线
                raise Exception(f"存在非法边，目标节点不存在: {source} -> {target} (source label: {sourceLabel})")  # 抛出非法边错误并附带源label
            continue  # 跳过这条边
    ...
    if len(result) != len(nodes):  # 如果结果数量不等于节点数量
        ...
        cycleNodesText = ",".join([f"{nodeId}({nodeLabels.get(nodeId, nodeId)})" for nodeId in cycleNodeIds])  # 把循环节点格式化为id(label)以便更快定位
        raise Exception(f"存在循环依赖，无法进行拓扑排序，涉及节点: {cycleNodesText}")  # 抛出包含节点id和label列表的异常
