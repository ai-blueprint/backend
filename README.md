...
| `label` | `泄漏ReLU` | 节点列表和画布 | 短而易懂的白话名称 |

例如，`leaky_relu`（显示为“泄漏ReLU”）的意思是：正数照原样通过，负数不再完全变成零，而是保留一个小斜率。标题保持短，完整解释放在 `description` 中。新节点只能使用 `opcode` 的规范格式；历史名称可以登记到 `registry.nodeAliases`，只负责兼容旧蓝图，不会出现在新节点列表中。
