...
async def runHotReload(broadcast, folder="nodes"):
    ...
    async for changes in awatch(nodesDir):  # 持续监听目录变化事件
        pyChanges = [
            item for item in changes if str(item[1]).endswith(".py")
        ]  # 仅处理python文件变化
        if not pyChanges:
        ...
        try:
            ...
            await broadcast(
                "registryUpdated", registry.getAllForFrontend()
            )  # 广播更新后的注册表给前端
        except Exception as error:
            print(f"热重载失败，回滚: {error}")  # 输出失败日志
            registry.nodes.clear()  # 清空当前节点注册表
            registry.categories.clear()  # 清空当前分类注册表
            registry.nodes.update(snapshot[0])  # 回滚节点快照
            registry.categories.update(snapshot[1])  # 回滚分类快照
